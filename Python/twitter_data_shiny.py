...
choices_check = ({'created_at': 'Date', 'text': 'Text', 'retweet_count': 'Retweets', 'favorite_count': 'Likes'})
    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ...
            ui.panel_conditional(
                ...
                ui.input_checkbox_group(id = "cols", 
...
def server(input, output, session):
    ...
    def bar():
        ...
        plot = (gg.ggplot(avg_int, gg.aes(input.x(), 'value', fill = 'variable')) +
                    ...
                    gg.theme_classic())
        if input.x() == 'day':
            return plot + gg.scale_x_discrete(labels = (["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]))
        else:
            return plot
    @output
    @render.text
    def text():
        if input.cols() == () or input.num() == 0 or input.num() < 0 or input.num() > 50:
            return ""
        elif input.num() == 1:
    ...
    def table():
        ...
        cols.rename(columns = {'created_at': 'Date', 'text': 'Text', 'retweet_count': 'Retweets', 'favorite_count': 'Likes'}, 
                    inplace = True)
